...
/obj/item/weapon/spellbook/attack_self(mob/user as mob)
	...
	if(temp)
		...
		dat += "<A href='byond://?src=\ref[src];spell_choice=curse'>Curse</A> (Variable)<BR>"
		dat += "<I>This spell allows you to imbue nearby items with curses.</I><BR>"
			...
			*/
				if(!already_knows)
					switch(href_list["spell_choice"])
						...
						if("curse")
							feedback_add_details("wizard_spell_learned","CU") //please do not change the abbreviation to keep data processing consistent. Add a unique id to any new spells
							H.mind.spell_list += new /obj/effect/proc_holder/spell/aoe_turf/curse/wizard(null)
							temp = "You have learned curse."
						if("horseman")
...
/obj/item/weapon/spellbook/lesser
	name = "Eldar Rune"
	desc = "A Wraithbone Eldar Rune."
	icon = 'icons/obj/wizard.dmi'
	icon_state ="Eldar Rune" //I am fully aware of what it is, It's all part of a master plan to make Linakus mad and make a new sprite for it. //Brilliant! Everything will go according to plan.
	throw_speed = 2
	throw_range = 5
	w_class = 1.0
	uses = 2
