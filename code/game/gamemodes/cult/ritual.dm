...
// join hide technology - stun rune. Rune color: bright pink.
	...
	attack_hand(mob/living/user as mob)		// OH GOD this is horrible
		...
		if(word1 == wordtravel && word2 == wordhell && word3 == wordtech)
			return summon_nspawn()
		if(word1 == wordsee && word2 == wordself && word3 == worddestr)
			return summon_borer()
		else
			user.take_overall_damage(30, 0)
			user << "\red You feel the life draining from you, as if Grandfather Nurgle is displeased with you."
			return fizzle()
	...
	proc
		fizzle()
			if(istype(src,/obj/effect/rune))
				usr.say(pick("...for Nurgle","Nurgle loves us","Nurgle loves you","Let my flesh fester and rot.","DO YOU HEAR THE VOICES TOO?!","It is a good pain...","Destroy the temples","In the embrace of the great Nurgle, I am no longer afraid.","Slay the priests"))
			else
				usr.whisper(pick("...for Nurgle","...Nurgle loves us","...Nurgle loves you","Let my flesh fester and rot.","Do you hear the voices too?","It is a good pain...","Destroy the temples","In the embrace of the great Nurgle, I am no longer afraid.", "Let us play hide and slay."))
...
/obj/item/weapon/tome
	...
	tomedat = {"<html>
				...
				<body>
				<h1>The scriptures of Nurgle, The Great One, The lover of man.</h1>
				...
				<b>Summon Nurgle: </b>Hell Join Self<br>
				...
				This rune opens target's mind to the realm of Nurgle, which usually results in this person joining the cult. However, some people (mostly the ones who posess high authority) have strong enough will to stay true to their old ideals. <br>
				<h3>Summon Nurgle</h3>
				The ultimate rune. It summons the Avatar of Nurgle himself, tearing a huge hole in reality and consuming everything around it. Summoning it is the final goal of any cult.<br>
				...
				<h3>Equip Armor</h3>
				When this rune is invoked, either from a rune or a talisman, it will equip the user with the armor of the followers of Nurgle. To use this rune to its fullest extent, make sure you are not wearing any form of headgear, armor, gloves or shoes, and make sure you are not holding anything in your hands.<br>
				<h3>See Invisible</h3>
				...
				switch(alert("You open the tome",,"Commune","Scribe a rune", "Notes")) //Fuck the "Cancel" option. Rewrite the whole tome interface yourself if you want it to work better. And input() is just ugly. - K0000
					...
					if("Scribe a rune")		//fixed more assbackward tabbing
						...
						var/list/dictionary = list(
							...
							"summon chaos" = list("hell","join","self"),
							...
							"summon nurgle spawn" = list("travel","hell","technology"),
							"summon borer" = list("see","self","destroy")
							)
						...
						if(do_after(user, 50))
							...
							user << "\red You finish drawing the arcane markings of Chaos."
	...
	examine()
		...
		if(!iscultist(usr))
			...
			usr << "The scriptures of Nurgle, The One Who Sees, The Lord of plagues. Contains the details of every ritual his followers could think of. Most of these are useless, though."
...
/obj/item/weapon/tome/imbued //admin tome, spawns working runes without waiting
	...
	attack_self(mob/user as mob)
		...
		if(user)
			...
			if (!istype(user.loc,/turf))
				user << "\red You do not have enough space to write a proper rune."
			var/list/runes = list("teleport", "itemport", "tome", "armor", "convert", "tear in reality", "emp", "drain", "seer", "raise", "obscure", "reveal", "astral journey", "manifest", "imbue talisman", "sacrifice", "wall", "freedom", "cultsummon", "deafen", "blind", "bloodboil", "communicate", "stun", "summon nurgle spawn", "summon borer")
