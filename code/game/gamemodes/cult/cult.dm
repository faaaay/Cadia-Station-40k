...
/datum/game_mode/cult
	...
	restricted_jobs = list("Chaplain","AI", "Cyborg", "Warden", "Detective", "Head of Security", "Captain", "Undercover Goose")
	protected_jobs = list()
	required_players = 6
	required_enemies = 6
...
/datum/game_mode/cult/proc/memorize_cult_objectives(var/datum/mind/cult_mind)
	for(var/obj_count = 1,obj_count <= cult_objectives.len,obj_count++)
		...
		switch(cult_objectives[obj_count])
			if("survive")
				explanation = "Chaos must live on. Make sure at least [acolytes_needed] acolytes escape on the shuttle to spread their work on an another station."
			if("sacrifice")
				if(sacrifice_target)
			...
			if("eldergod")
				explanation = "Summon a Nurgle Daemon via the use of the appropriate rune (Hell join self). It will only work if nine cultists stand on and around it."
		cult_mind.current << "<B>Objective #[obj_count]</B>: [explanation]"
		cult_mind.memory += "<B>Objective #[obj_count]</B>: [explanation]<BR>"
	cult_mind.current << "Nurgle grants you the knowledge to sacrifice non-believers. (Hell Blood Join)"
	cult_mind.memory += "Nurgle grants you the knowledge to sacrifice non-believers. (Hell Blood Join)<BR>"
...
/datum/game_mode/proc/grant_runeword(mob/living/carbon/human/cult_mob, var/word)
	...
	if(cult_mob.mind.cult_words.len == allwords.len)
		cult_mob << "\green You feel enlightened, as if you have gained all the secrets of the warp."
...
/datum/game_mode/proc/add_cultist(datum/mind/cult_mind) //BASE
	...
	if(!(cult_mind in cult) && is_convertable_to_cult(cult_mind))
		...
		update_cult_icons_added(cult_mind)
		cult_mind.current.attack_log += "\[[time_stamp()]\] <font color='red'>Has been converted to chaos!</font>"
		return 1
...
/datum/game_mode/proc/remove_cultist(datum/mind/cult_mind, show_message = 1)
	if(cult_mind in cult)
		...
		cult_mind.current << "\red <FONT size = 3><B>An unfamiliar white light flashes through your mind, cleansing the taint of Nurgle and the memories of your time as his servant with it.</B></FONT>"
		cult_mind.memory = ""
		cult_mind.cult_words = initial(cult_mind.cult_words)
		update_cult_icons_removed(cult_mind)
		cult_mind.current.attack_log += "\[[time_stamp()]\] <font color='red'>Has renounced Nurgle!</font>"
		if(show_message)
...
/datum/game_mode/cult/declare_completion()
	...
	if(!check_cult_victory())
		...
		world << "\red <FONT size = 3><B> The cultists win! They have succeeded in serving Nurgle!</B></FONT>"
	else
		feedback_set_details("round_end_result","loss - staff stopped the cultists")
		feedback_set("round_end_result",acolytes_survived)
		world << "\red <FONT size = 3><B> The staff managed to stop the cultists!</B></FONT>"
	...
	if(cult_objectives.len)
		...
		for(var/obj_count=1, obj_count <= cult_objectives.len, obj_count++)
			...
			switch(cult_objectives[obj_count])
				...
				if("eldergod")
					if(!eldergod)
						explanation = "Summon Nurgle Daemon. <font color='green'><B>Success!</B></font>"
						feedback_add_details("cult_objective","cult_narsie|SUCCESS")
					else
						explanation = "Summon Nurgle Daemon. <font color='red'>Fail.</font>"
