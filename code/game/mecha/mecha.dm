...
/obj/mecha
	...
	//inner atmos //NOBODY USES THIS WHY TG WHY
	var/use_internal_tank = 0
	//var/internal_tank_valve = ONE_ATMOSPHERE
	//var/obj/machinery/portable_atmospherics/canister/internal_tank
	//var/datum/gas_mixture2/cabin_air
	//var/obj/machinery/atmospherics/pipe/canister_connector/connected_port = null
	...
	var/can_drop = 1 //If it can drop equipment
...
/obj/mecha/hear_talk(mob/M as mob, text)
	...
	if(M==passenger && radio.broadcasting)
		radio.talk_into(M, text)
	return
...
/obj/mecha/proc/setInternalDamage(int_dam_flag)
	...
	passenger << sound('sound/machines/warning-buzzer.ogg',wait=0)
	return
...
/obj/mecha/attack_alien(mob/user as mob)
	...
	if(istype(user, /mob/living/carbon/alien/humanoid/tyranid/ravener))
		var/mob/living/carbon/alien/humanoid/tyranid/ravener/R = user
		if(R.rending_claws)
			if(istype(src, /obj/mecha/combat/killakan)) //an anti-armor unit needs to be able to destroy a junkpile mech... Which happens to have 20,000 health.
				src.take_damage(200) //Thus a buff on damage dealt is in order. Now a few hits from a ravener will actually destroy one of them.
			if(istype(src, /obj/mecha/combat/crisessuit)) //More balancing against mecha with heretical levels of health.
				src.take_damage(200)
			if(istype(src, /obj/mecha/combat/wraithlord))
				src.take_damage(200)
			src.take_damage(65) //Sentinel: 4 hits, Dreadnought: 16 hits, Killakan: 8 hits, chimera: 6 hits, crisis suit: 16 hits, wraithlord: 16 hits
			src.check_for_internal_damage(list(MECHA_INT_TEMP_CONTROL,MECHA_INT_TANK_BREACH,MECHA_INT_CONTROL_LOST))
			playsound(src.loc, 'sound/weapons/slash.ogg', 50, 1, -1)
			user << "\red You rend the armored suit!"
			visible_message("\red The [user] rends [src.name]'s armor!")
			return
	if(istype(user, /mob/living/carbon/alien/humanoid/tyranid/lictor))
		var/mob/living/carbon/alien/humanoid/tyranid/lictor/L = user
		if(L.piercing)
			if(istype(src, /obj/mecha/combat/killakan))
				src.take_damage(200)
			if(istype(src, /obj/mecha/combat/crisessuit))
				src.take_damage(200)
			if(istype(src, /obj/mecha/combat/wraithlord))
				src.take_damage(200)
			src.take_damage(65)
			src.check_for_internal_damage(list(MECHA_INT_TEMP_CONTROL,MECHA_INT_TANK_BREACH,MECHA_INT_CONTROL_LOST))
			playsound(src.loc, 'sound/weapons/slash.ogg', 50, 1, -1)
			user << "\red You pierce the armored suit!"
			visible_message("\red The [user] pierces [src.name]'s armor!")
			return
...
/obj/mecha/proc/dynbulletdamage(var/obj/item/projectile/Proj)
	...
	if((Proj.damage_type == BRUTE || Proj.damage_type == BURN))
		src.take_damage(Proj.damage+Proj.piercing,Proj.flag)
...
/obj/mecha/temperature_expose(datum/gas_mixture2/air, exposed_temperature, exposed_volume)
...
/obj/mecha/proc/dynattackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/chainsword))
		...
		var/Wforce = W.force*2
		if(ishuman(user))
			var/mob/living/carbon/human/H = user
			if(H.inertial_speed != null)
				Wforce += (H.inertial_speed*5)
				H << "\red You use your momentum to deal a more forceful blow."
		src.take_damage(Wforce)
		src.check_for_internal_damage(list(MECHA_INT_TEMP_CONTROL,MECHA_INT_TANK_BREACH,MECHA_INT_CONTROL_LOST))
		return
	if(prob(src.deflect_chance))
...
/obj/mecha/proc/get_pressure()
	var/datum/gas_mixture2/t_air = get_air()
	if(t_air)
		. = t_air.get_pressure()
	return
...
/*
/obj/mecha/proc/connect(obj/machinery/atmospherics/pipe/canister_connector/new_port)
...
/obj/mecha/verb/start_move_inside()
	set category = "Object"
	set name = "Enter Exosuit"
	set src in oview(1)
	move_inside(usr)

/obj/mecha/proc/move_inside(var/mob/usr)
	...
	if(H && H.client && H in range(1))
		...
		*/
		...
		if(!hasInternalDamage())
			playsound(src,nominalsound,40,0)
		return 1
	else
		return 0
...
/obj/mecha/proc/dump_pass()
	if(!src.passenger) return
	var/atom/movable/mob_container
	if(ishuman(passenger))
		mob_container = src.passenger
	else if(istype(passenger, /mob/living/carbon/brain))
		var/mob/living/carbon/brain/brain = passenger
		mob_container = brain.container
	else
		return
	if(mob_container.forceMove(src.loc))//ejecting mob container
...
/obj/mecha/proc/report_internal_damage()
	...
	for(var/tflag in dam_reports)
		...
		if(hasInternalDamage(intdamflag))
			...
			output += "<br />"
	if(get_pressure() > WARNING_HIGH_PRESSURE)
...
/obj/mecha/proc/get_stats_part()
	...
	//var/tank_pressure = internal_tank ? round(internal_tank.get_pressure(),0.01) : "None"
	//var/tank_temperature = internal_tank ? internal_tank.return_temperature() : "Unknown"
	var/cabin_pressure = round(get_pressure(),0.01)
...
/obj/mecha/proc/get_equipment_menu() //outputs mecha html equipment menu
	...
	if(equipment.len)
		...
		for(var/obj/item/mecha_parts/mecha_equipment/W in equipment)
			output += "[W.name]"
			if(can_drop) output += "<a href='?src=\ref[W];detach=1'>Detach</a><br>"
...
/datum/global_iterator/mecha_preserve_temp  //normalizing cabin air temperature to 20 degrees celsium
	...
	process(var/obj/mecha/mecha)
		//if(mecha.cabin_air && mecha.cabin_air.get_pressure() > 0)
...
/datum/global_iterator/mecha_internal_damage // processing internal damage
	...
	process(var/obj/mecha/mecha)
		...
		if(mecha.hasInternalDamage(MECHA_INT_FIRE))
			...
			//	if(mecha.internal_tank.get_pressure()>mecha.internal_tank.maximum_pressure && !(mecha.hasInternalDamage(MECHA_INT_TANK_BREACH)))
			//		mecha.setInternalDamage(MECHA_INT_TANK_BREACH)
			//	var/datum/gas_mixture2/int_tank_air = mecha.internal_tank.get_air()
			//	if(int_tank_air && int_tank_air.get_pressure()>0) //heat the air_contents
		...
		//if(mecha.hasInternalDamage(MECHA_INT_TANK_BREACH)) //remove some air from internal tank
			...
			//	var/datum/gas_mixture2/int_tank_air = mecha.internal_tank.get_air()
			//	if(int_tank_air)
			//		var/datum/gas_mixture2/leaked_gas = int_tank_air.remove_ratio(0.10)
