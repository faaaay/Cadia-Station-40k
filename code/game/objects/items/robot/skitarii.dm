...
/obj/item/skitarii/l_arm
	name = "skitarii left arm"
	desc = "A skeletal limb wrapped in pseudomuscles, with a low-conductivity case."
	icon_state = "l_arm"
	construction_time = 200
	construction_cost = list("metal"=10000)

/obj/item/skitarii/r_arm
	name = "skitarii right arm"
	desc = "A skeletal limb wrapped in pseudomuscles, with a low-conductivity case."
	icon_state = "r_arm"
	construction_time = 200
	construction_cost = list("metal"=10000)

/obj/item/skitarii/l_leg
...
/obj/item/skitarii/robot_suit
	name = "skitarii endoskeleton"
	desc = "A complex metal backbone with standard limb sockets and pseudomuscle anchors."
	icon_state = "robo_suit"
	construction_time = 500
	construction_cost = list("metal"=15000)
	var/obj/item/skitarii/l_arm/l_arm = null
	var/obj/item/skitarii/r_arm/r_arm = null
	var/obj/item/skitarii/l_leg/l_leg = null
	var/obj/item/skitarii/r_leg/r_leg = null
	var/obj/item/skitarii/chest/chest = null
	var/obj/item/skitarii/head/head = null
...
/obj/item/skitarii/robot_suit/New()
	..()
	src.updateicon()

/obj/item/skitarii/robot_suit/proc/updateicon()
	src.overlays.Cut()
	if(src.l_arm)
...
/obj/item/skitarii/skitarii/proc/check_completion()
...
/obj/item/skitarii/skitarii/attackby(obj/item/W as obj, mob/user as mob)
	..()
	if(istype(W, /obj/item/stack/sheet/metal) && !l_arm && !r_arm && !l_leg && !r_leg && !chest && !head)
		var/obj/item/weapon/ed209_assembly/B = new /obj/item/weapon/ed209_assembly
		B.loc = get_turf(src)
		user << "You armed the skitarii frame"
		W:use(1)
		if (user.get_inactive_hand()==src)
			user.unEquip(src)
			user.put_in_inactive_hand(B)
		qdel(src)
	if(istype(W, /obj/item/skitarii/l_leg))
		if(src.l_leg)	return
		user.drop_item()
		W.loc = src
		src.l_leg = W
	...
	if(istype(W, /obj/item/skitarii/r_leg))
		if(src.r_leg)	return
		user.drop_item()
		W.loc = src
		src.r_leg = W
	...
	if(istype(W, /obj/item/skitarii/l_arm))
		if(src.l_arm)	return
		user.drop_item()
		W.loc = src
		src.l_arm = W
	...
	if(istype(W, /obj/item/skitarii/r_arm))
		if(src.r_arm)	return
		user.drop_item()
		W.loc = src
		src.r_arm = W
	...
	if(istype(W, /obj/item/skitarii/chest))
		if(src.chest)	return
		if(W:wires && W:cell)
			user.drop_item()
			W.loc = src
	...
	if(istype(W, /obj/item/skitarii/head))
		if(src.head)	return
...
/obj/item/skitarii/robot_suit/proc/Interact(mob/user)
...
/obj/item/skitarii/robot_suit/Topic(href, href_list)
...
/obj/item/skitarii/chest/attackby(obj/item/W as obj, mob/user as mob)
	..()
...
/obj/item/skitarii/head/attackby(obj/item/W as obj, mob/user as mob)
	..()
