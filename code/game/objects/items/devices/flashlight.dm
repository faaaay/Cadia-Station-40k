...
/obj/item/device/flashlight/seclite
	...
	icon_state = "seclite"
	item_state = "seclite"
	force = 1 // Not as good as a stun baton.
...
/obj/item/device/flashlight/flare2
	name = "flare"
	desc = "It's a flare. I'm no scientist but it probably means humans are nearby."
	icon_state = "flare2"
	item_state = "flare2"
	w_class = 2.0
	brightness_on = 7 // Pretty bright.
...
/obj/item/device/flashlight/flare2/New()
	fuel = rand(800, 1000) // Sorry for changing this so much but I keep under-estimating how long X number of ticks last in seconds.
	on = 1
	update_brightness(7)
	..()

/obj/item/device/flashlight/flare2/process()
	var/turf/pos = get_turf(src)
	if(pos)
		pos.hotspot_expose(produce_heat, 5)
	fuel = max(fuel - 1, 0)
...
/obj/item/device/flashlight/flare2/proc/turn_off()
	on = 0
	src.force = initial(src.force)
	src.damtype = initial(src.damtype)
	if(ismob(loc))
...
/obj/item/device/flashlight/flare2/attack_self(mob/user)
	...
	// All good, turn it on.
	if(.)
		user.visible_message("<span class='notice'>The flare sputters to life.</span>", "<span class='notice'>The flare sputters to life.</span>")
		src.force = on_damage
		src.damtype = "fire"
		processing_objects += src
