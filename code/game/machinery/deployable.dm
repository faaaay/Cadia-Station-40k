/*
TODO:
make trenches that make somebody not have density (harder to hit)
make barricades that have a chance to block attacks going in one direction
...
/obj/structure/barricade/metal //This will be an effective defense measure and should totally be made a thing.
	name = "barricade"
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/stack/sheet/metal))
			...
			if (src.health <= 0)
				visible_message("\red <B>The barricade is destroyed!</B>")
...
/obj/structure/barricade/barbedwire
	name = "barbed wire"
	desc = "The perimeter of the outpost, maintained by a barbed wire fence."
	icon = 'icons/obj/structures.dmi'
	...
	blob_act()
		src.health -= 25
		if (src.health <= 0)
			visible_message("\red <B>The blob eats through the barbed wire!</B>")
			qdel(src)
		return
