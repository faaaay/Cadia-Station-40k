...
//				text += "<br>All the changelings are dead! Restart in [round((changeling.TIME_TO_GET_REVIVED-(world.time-changeling.changelingdeathtime))/10)] seconds."
	...
	Topic(href, href_list)
		...
		if (href_list["role_edit"])
			...
			switch(href_list["wizard"])
				if("clear")
					remove_antag("Wizard")
					current << "\red <FONT size = 3><B>You have been brainwashed! You are no longer an Eldar!</B></FONT>"
					log_admin("[key_name(usr)] has de-warlock'ed [current].")
				if("wizard")
					if(!(src in ticker.mode.wizards))
						ticker.mode.wizards += src
						special_role = "Wizard"
						//ticker.mode.learn_basic_spells(current)
						current << "<B>\red You are an Eldar Warlock!</B>"
						message_admins("[key_name_admin(usr)] has warlock'ed [current].")
						log_admin("[key_name(usr)] has warlock'ed [current].")
				if("lair")
				...
				if("autoobjectives")
					...
					usr << "\blue The objectives for warlock [key] have been generated. You can edit them and anounce manually."

		else if (href_list["changeling"])
			switch(href_list["changeling"])
				if("clear")
					remove_antag("Changeling")
					current << "<FONT color='red' size = 3><B>Turns out... you only THOUGHT you were a Callidus assassin. As it turns out- not so much.</B></FONT>"
					message_admins("[key_name_admin(usr)] has de-assassined [current].")
					log_admin("[key_name(usr)] has de-assassined [current].")
				if("changeling")
					if(!(src in ticker.mode.changelings))
						ticker.mode.changelings += src
						current.make_changeling()
						special_role = "Changeling"
						current << "<B><font color='red'>A flash of memory returns to us...Holy Crap! You are a Callidus Assassin!</font></B>"
						message_admins("[key_name_admin(usr)] has assassined'ed [current].")
						log_admin("[key_name(usr)] has assassin'ed [current].")
				if("autoobjectives")
					ticker.mode.forge_changeling_objectives(src)
					usr << "\blue The objectives for assassin [key] have been generated. You can edit them and anounce manually."
			...
			switch(href_list["nuclear"])
				if("clear")
					remove_antag("NuclearOp")
					current << "\red <FONT size = 3><B>You have been brainwashed! You are no longer part of the bloodpact!</B></FONT>"
				...
				if("nuclear")
					if(!(src in ticker.mode.syndicates))
						...
						if (ticker.mode.syndicates.len==1)
							...
							current.real_name = "Officer [ticker.mode.syndicates.len-1]"
						special_role = "Syndicate"
						current << "\blue You are part of the bloodpact!"
				...
				if("tellcode")
					...
					for (var/obj/machinery/nuclearbomb/bombue in world)
						if (length(bombue.r_code) <= 5 && bombue.r_code != "LOLNO" && bombue.r_code != "ADMIN")
							...
							break
					if (code)
						store_memory("<B>Imperial Nuclear Bomb Code</B>: [code]", 0, 0)
			...
			switch(href_list["traitor"])
				if("clear")
					...
					current << "\red <FONT size = 3><B>You have been brainwashed! You are no longer a Heretic!</B></FONT>"
				...
				if("traitor")
					if(!(src in ticker.mode.traitors))
						...
						current << "<B>\red You are a Heretic!</B>"
			...
			switch(href_list["common"])
				...
				if("crystals")
					if (check_rights(R_FUN, 0))
						...
						if (suplink)
							crystals = suplink.uses
						crystals = input("Amount of telecrystals for [key]","Warp uplink", crystals) as null|num
						if (!isnull(crystals))
							if (suplink)
	...
	proc/make_SOB()
		...
		special_role = "SOB"
		current << "\blue You are a Sister Of Battle!"

		current.loc = get_turf(locate("landmark*sobteam"))

	proc/make_UM()

		special_role = "UM"
		current << "\blue You are an UltraMarine!"
	...
	proc/make_Nuke()
		if(!(src in ticker.mode.syndicates))
			...
			if (ticker.mode.syndicates.len==1)
				...
				current.real_name = "Khorne Berseker #[ticker.mode.syndicates.len-1]"
			special_role = "Syndicate"
			assigned_role = "MODE"
			current << "\blue You are a Khorner Berseker!"
			ticker.mode.forge_syndicate_objectives(src)
			ticker.mode.greet_syndicate(src)
