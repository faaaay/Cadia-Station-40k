...
/proc/isork(A)
	if(istype(A, /mob/living/carbon/human/ork))
		return 1
	return 0

/proc/istau(A)
	if(istype(A, /mob/living/carbon/human/tau))
		return 1
	return 0

/proc/isgretchin(A)
	if(istype(A, /mob/living/carbon/human/ork/gretchin))
		return 1
	return 0
...
/mob/verb/a_intent_change(input as text)
	...
	if(ishuman(src) || isalienadult(src) || isork(src) || isgretchin(src) || isbrain(src))
		switch(input)
