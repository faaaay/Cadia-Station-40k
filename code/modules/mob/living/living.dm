/mob/living/Life()
	...
	if(src.flying)
		if(src.dropping)
			if(istype(src.loc, /turf/snow) || istype(src.loc, /turf/unsimulated/floor/snow))
				src.drop()
	if(src.ckey && src.client) //If this is actually a real person who is fully aware and all that.
		for(var/mob/living/simple_animal/SM in range((src.client.view+1), src))
			if(SM.asleep)
				SM.wake_up()
				SM.asleep = 0
...
/mob/living/proc/mob_sleep()
	...
	if(usr.sleeping)
		...
		if(alert(src, "You sure you want to sleep for a while?", "Sleep", "Yes", "No") == "Yes")
			usr.sleeping = 40 //Short nap //Doubled length because this is just too short -Drake
...
/mob/living/proc/lay_down()
	...
	update_canmove() //Now this is actually robust and useful for ducking! -Drake
...
/mob/living/verb/resist()
	...
	//Stop drop and roll & Handcuffs
		...
		if(CM.handcuffed && CM.canmove && (CM.last_special <= world.time))
			...
			if(isalienadult(CM) || (isork(CM)) || (HULK in usr.mutations))//Don't want to do a lot of logic gating here.
