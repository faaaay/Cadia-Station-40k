/mob/living/proc/run_armor_check(def_zone = null, attack_flag = "melee", absorb_text = null, soften_text = null)
	...
	if(armor > 0 && luck != 0) //A little trick to make armor act better or worse by fate...
		armor += round(luck/10)
	if(armor >= 100)
		if(absorb_text)
...
/mob/living/bullet_act(obj/item/projectile/P, def_zone, var/has_limbs)
	...
	armor = max(armor - P.piercing, 0) //If the projectile pierces armor. Default piercing is 0.
	if(!P.nodamage)
		if(ishuman(src))
			apply_damage(P.damage, P.damage_type, def_zone, armor, P.woundtype)
		else
...
/mob/living/proc/handle_fire()
	...
	if(!on_fire)
		return 1
	var/datum/gas_mixture2/G = loc.get_air() // Check if we're standing in an oxygenless environment
	if(G.oxygen < 1)
